    ...
    companion object {
        ...
        fun desdeHex(hexString: String): Color? {
            try {
                ...
                when (hex.length) {
                    3 -> {
                        val r = Integer.parseInt("${hex[0]}${hex[0]}", 16)
                        val g = Integer.parseInt("${hex[1]}${hex[1]}", 16)
                        val b = Integer.parseInt("${hex[2]}${hex[2]}", 16)
            ...
            try {
                ...
                if (parts.size == 3 || parts.size == 4) {
                    ...
                    val a = if (parts.size == 4) {
                        ...
                        if (alpha <= 1f) (alpha * 255).toInt().coerceIn(0, 255) else alpha.toInt().coerceIn(0, 255)
                    } else 255
                    return Color(r, g, b, a)
                }
            } catch (_: Exception) {}
        ...
        fun desdeHsl(hslString: String): Color? {
            try {
                val str = hslString.trim().let {
                    if (it.startsWith("<") && it.endsWith(">")) {
                        it.substring(1, it.length - 1)
                    } else if (it.startsWith("hsl(") && it.endsWith(")")) {
                        it.substring(4, it.length - 1)
                    } else {
                        it
                    }
                }
                ...
                if (parts.size == 3) {
                    ...
                    val sPct = parts[1].toFloatOrNull() ?: return null
                    val lPct = parts[2].toFloatOrNull() ?: return null
